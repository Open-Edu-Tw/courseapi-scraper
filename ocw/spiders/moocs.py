...
class MoocsSpider(OCWScraper):
    ...
    def parse_course_directoy(self, response):
        ...
        if "page" not in response.meta:  # run only at first page
            last_page_num = int(response.xpath("//a[@data-ci-pagination-page and contains(text(), 'Last')]/@data-ci-pagination-page").get())
            for p in range(1, last_page_num + 1):
                yield scrapy.Request(f"{url}/{len(courses) * p}", callback=self.parse_course_directoy, meta={"page": p})

    def parse_course(self, response):
        course_item = CourseItem(name=response.xpath("//h1/text()").get(),
                                 url=response.url,
